  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
        Somente leitura. Cargas novas rodam no servidor com{" "}
        <code className="rounded bg-muted px-1">php tools/import-companies.php</code>
        . Não há botão para executar importação pela web.
      </p>
      {q.data?.not_migrated ? (
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-3">Inseridos</th>
              <th className="px-4 py-3">Atualizados</th>
              <th className="px-4 py-3">Ignorados</th>
            ...
            {runs.length === 0 ? (
              ...
              runs.map((r) => (
                <tr key={r.id} className="border-t border-border align-top">
                  ...
                  <td className="px-4 py-3">{r.dry_run ? "sim" : "não"}</td>
                  <td className="px-4 py-3">{r.total_collected}</td>
                  <td className="px-4 py-3">{r.total_inserted}</td>
                  <td className="px-4 py-3">{r.total_updated}</td>
                  <td className="px-4 py-3">{r.total_skipped + r.total_duplicates}</td>
